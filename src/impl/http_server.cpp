...
{
  auto* parser = static_cast<HttpServer*>(http);
...
SessionPtr
HttpServer::create(MessagePtr message)
{
  return SessionPtr();
}
...
void
HttpServer::set_field(std::string_view name, std::string_view value)
{
...
void
HttpServer::handle_error(llhttp_status_t status)
{
  std::string response;
  const auto& request_body = request_->get_body();
  response += "HTTP/";
...
void
HttpServer::handle_success(MessagePtr message)
{
  std::string response;
